...
{
public:
    virtual void applyFilter(ImageRGBU8 &image) const = 0;
};
...
public:
    void applyFilter(ImageRGBU8 &image) const ;
...
private:
    ...
    Filter **f; // pointeur vers un tableau de filter
public:
    void addFilter(Filter &filter);
    void applyFilters(ImageRGBU8 &image);
    Filters(unsigned int taille);
    ~Filters();
